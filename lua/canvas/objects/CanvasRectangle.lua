...
local CanvasRectangle = ErrorOnInvalidRead.new{
    ...
    makeFromInitData = function(initData)
        return AbstractCanvasObject.new({
            id = (rendering.draw_rectangle(initData)).id,
            type = "rectangle",
        }, Metatable)
    end,
...
Metatable = {
    __index = {
        ...
        isCollidingWithAabb = function(self, aabb)
            local id = self.id
            local ltPos = rendering.get_object_by_id(id).left_top.position
            local rbPos = rendering.get_object_by_id(id).right_bottom.position
